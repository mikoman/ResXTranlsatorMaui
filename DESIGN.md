...
- Ollama and LM Studio are Mac Catalyst-only presets with editable loopback/private-network endpoints. The app declares local-network access and a narrow ATS local-network exception, and never discovers or manages inference servers.
...
The original bounded finish review used cropped screenshots from a locally built, ad-hoc-signed Mac Catalyst Debug app on 1 September 2026. Those captures predate multi-provider support and are not shipping assets or repository content. iOS and Android remain compile-only validation targets; no device-level visual claim is made for them.
