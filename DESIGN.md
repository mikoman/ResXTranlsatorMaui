...
ResXTranslator is a compact native utility, not a dashboard or a web form. Its visual language follows Apple's grouped settings surfaces: semantic system-like backgrounds, inset row cards, hairline separators, SF Symbols, restrained system typography, and one violet primary action. The Mac Catalyst window uses a roomier `720×680` working size so four-request translation progress remains visible without routine scrolling; unusually long content and focused setup tasks still scroll inside the window.
...
- Progress is both determinate and active: the title reports the number of active OpenRouter requests while supporting text moves through request sent, provider connected, response streaming, validation, and file-writing stages. It includes completed entries, completed and total batches with a percentage, per-batch retry attempts, request IDs shared with the redacted diagnostics log, streamed response size, and elapsed time. The pulse becomes static when Reduce Motion is enabled.
- Concurrent progress remains one compact status group: the title reports active-request count, the summary reports entries/batches and the 2× or 4× limit, and up to four short request lines expose file, batch, phase, response size, and elapsed time.
